...
char *execute_cmd(commande cmd, Reservation *reservation)
{
    ...
    {
        strcat(res, "alloc cpu mem:                   Allouer'cpu' cpu et 'mem' memory sur n'importe quel serveur\n");
        strcat(res, "alloc (server_name cpu mem)+...: Allouer 'cpu' cpu et 'mem' memory sur le serveur 'server_name'\n");
        strcat(res, "free (server_name)*:             Libérer tous ce qui a été alloué sur les serveurs spécifiés\n");
        strcat(res, "free:                            Libérer toutes les ressources reserver.\n");
        strcat(res, "exit:                            Quitter proprement\n");
    }
    else
    {
        //je sais pas quoi mettre la mdr
    }
    return res;
}
...
int manage_user(int ds, int ds_client, struct sockaddr_in addr_client, socklen_t socklen, int port, struct recv *r)
{
    ...
    while (1)
    {
        ...
        commande cmd = interpret_cmd(in_buffer);
        in_buffer[0] = '\0';
        in_buffer = execute_cmd(cmd, reservation);
        if (strcmp(in_buffer, "") == 0)
            strcpy(in_buffer, "Erreur à l'éxecution de la commande");
        if (cmd.cmd_type == CMD_EXIT)
        {
            strcpy(in_buffer, "exit");
            if (sendTCP(ds_client, in_buffer, 500) == -1)
            {
                perror("Can't send the message:");
            }
            break;
